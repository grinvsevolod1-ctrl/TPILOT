...
def _psb_utc_now():
    """Naive-UTC clock seam (utcnow refactor stage 2, 2026-08-16). Same
    contract as main._tp_utc_now() / storage._utc_now(): returns what
    _psb_utc_now() used to return (a NAIVE datetime in UTC), so every
...
def _now_iso() -> str:
    return _psb_utc_now().replace(microsecond=0).isoformat()
...
def _pse_record_failure(uid: int, event_id: int, exc: BaseException) -> None:
    ...
    try:
        ...
        if dead:
            ...
            next_attempt_at = (_psb_utc_now() + timedelta(
...
def _arn_utc_iso_to_kyiv_hm(iso_utc: str) -> str:
    ...
    (naive UTC, no tz suffix, microsecond=0 -- what datetime.utcnow()
    used to produce; see the _psb_utc_now seam) to a Kyiv HH:MM display
    string. Never raises."""
...
def _rsvan_done_events_for_source(source_key: str, *, baseline_ts: str,
    ...
    if not sk or not baseline_ts:
        return []
    cutoff_iso = (_psb_utc_now().replace(microsecond=0) - timedelta(hours=int(freshness_hours))).isoformat()
    try:
        storage.ensure_reserve_tables(TPILOT_DB_PATH)
        con = sqlite3.connect(TPILOT_DB_PATH)
        con.row_factory = sqlite3.Row
