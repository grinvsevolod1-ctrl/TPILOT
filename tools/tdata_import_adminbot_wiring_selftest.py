...
def main():
    ...
    # File-copy backups belong to the ORIGINAL no-git workflow; they exist
    # only on the machine where the phase patch was applied and were never
    # committed. On a git checkout (repo migrated 2026-08) git history is
    # the provenance, so skip the presence gate there; keep it hard for
    # non-git trees.
    import glob
    backups = glob.glob(os.path.join(BASE_DIR, "main.py.bak_tdimport_*"))
    if not backups and os.path.isdir(os.path.join(BASE_DIR, ".git")):
        print("    (note) no main.py.bak_tdimport_* backup -- git checkout, "
              "history is the provenance; skipping backup-presence gate.")
    else:
    ...
    # PREPARED ACCOUNTS PHASE 5 refactored the literal kwarg into a dict item
    # (set_fields_kwargs["auth_profile"] = "project") gated ONLY by the new
    # preserve_auth_profile flag (default False -> reset still happens on
    # every ordinary phone/QR/relogin/replacement finalize; True is used
    # exclusively by Phase 5 activation of imported 'tdesktop' managers).
    finalize_src = _src_without_docstring(last_def(tree, "_manager_finalize_login"))
    _reset_forms = (
        "auth_profile='project'" in finalize_src
        or 'auth_profile="project"' in finalize_src
        or '"auth_profile"] = "project"' in finalize_src
        or "'auth_profile'] = 'project'" in finalize_src
    )
    check("_manager_finalize_login resets auth_profile='project' on every successful "
          ...
          _reset_forms and "manager_set_fields" in finalize_src,
          detail=finalize_src[:300])
    # ast.unparse normalizes kwarg spacing to "name: bool=False"
    _finalize_unparsed = ast.unparse(last_def(tree, "_manager_finalize_login"))
    check("the auth_profile reset is gated only by preserve_auth_profile "
          "(default False keeps the reset-on-every-finalize invariant)",
          "preserve_auth_profile" in finalize_src
          and ("preserve_auth_profile: bool=False" in _finalize_unparsed
               or "preserve_auth_profile: bool = False" in _finalize_unparsed),
          detail=_finalize_unparsed[:200])
