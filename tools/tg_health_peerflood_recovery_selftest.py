...
def test_m_def_counts() -> None:
    ...
    # was 4 (3 dead + 1 active); commit ef4fb8d removed one DEAD override.
    # R1 BATCH 1 20260823: 3 -> 1 (the two remaining provably-dead shadowed
    # defs removed by tools/collapse_dead_defs.py; the active thin wrapper
    # over _send_manager_private_ex is unchanged).
    check("M2. _send_manager_private has exactly 1 definition (the active thin wrapper)", len(send_defs) == 1, len(send_defs))
...
async def test_abc_gate_and_recovery_primitives() -> None:
    ...
    for _seam in ("_tp_utc_now", "_mbot_utc_now", "_psb_utc_now", "_pbr_utc_now"):
        namespace.setdefault(_seam, lambda: __import__("datetime").datetime.now(
