...
def build_qr_ns(db_path: str, base_dir: Path, *, script: dict = None, exclude_names: set = None,
    ...
    ns = {
        ...
        **ast_extract.safe_module_ns(),
        ...
        # main.py binds _PREPARED_READY_MARKER at module level from
        # features/prepared_accounts/model.py (PREPARED ACCOUNTS PHASE 4/5);
        # _manager_qr_wait_task now checks the finalize result text for it.
        # Bind the REAL constant so the test stays in sync with production.
        "_PREPARED_READY_MARKER": __import__(
            "features.prepared_accounts.model", fromlist=["PREPARED_READY_MARKER"]
