...
def build_pure_ns() -> dict:
    ...
    ns = {"Dict": dict, "Any": object, "datetime": datetime, "timezone": timezone,
          # utcnow refactor (2026-08-16): freshness math reads the clock via
          # the module-level _pb_utc_now() seam (naive UTC contract).
...
def build_live_ns(db_path: str, proc_scan=None) -> dict:
    ...
    ns = {
        ...
        # utcnow refactor (2026-08-16): same _pb_utc_now() seam as above.
        "_pb_utc_now": (lambda: datetime.now(timezone.utc).replace(tzinfo=None)),
